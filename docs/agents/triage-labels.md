...
The skills speak in terms of five canonical triage roles. This file maps those roles to the actual label strings used in this repo's issue tracker (GitHub Issues on `zero-koo/zk-note`).
...
Of these, only `wontfix` exists on the repo today (it ships with GitHub's default label set). Create the other four on first use:
